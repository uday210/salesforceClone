  ...
  return (
    <div className="page">
      ...
      <div className="flex items-center mb" style={{ justifyContent: "space-between" }}>
        <span className="muted">{records.length} item{records.length !== 1 ? "s" : ""}</span>
      </div>

      {mode === "kanban" && kanbanField ? (
        <KanbanBoard object={object} field={kanbanField} records={records} />
      ) : (
        <DataTable object={object} fields={fields} records={records} columns={activeView?.columns || []} />
      )}
    </div>
  );
}
